...
@section('content')
    ...
    <div class="row page-titles">
        ...
        <div class="col-md-7 align-self-center">
            {{ Breadcrumbs::render('backend.products.attributes.index') }}
        </div>
                    ...
                    <div class="row">
                        <div class="col-md-8 offset-2">
                            <menu id="nestable-menu" style="padding: 0">
                                @if(auth()->guard('backend')->user()->can('products.attributes.add')||1)
                                    <a href="{{Route('backend.products.attributes.add')}}" class="btn flat btn-info">
                                        Thêm mới
                                    </a>
                                @endif
                            </menu>
                                    ...
                                    @foreach($list_data as $v)
                                        <li class='dd-item' data-id='{{$v->id}}'>
                                            ...
                                            <div class='ddmenu-right button-group'>
                                                ...
                                                <a href='{{route('backend.products.attributes.values.index',$v->id)}}' class='status-button btn btn-xs btn-outline-primary'>
                                                    <i class='mdi mdi-format-list-bulleted'> </i>
                                                </a>

                                                <a href='{{route('backend.products.attributes.edit',$v->id)}}' class='btn btn-xs btn-outline-info'>
                            ...
                            <div class="text-center d-none">
                                <div class="btn-group ">
                                    <button type="button" id="save"
                                            class="btn btn-primary btn-flat"><i class="fa fa-save"></i> &nbsp; Lưu
                                    </button>
                                </div>
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            function sort_ajax() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('backend.products.attributes.sort')}}",
                    data: dataString,
                    cache: false,
                    dataType: 'json',
            ...
            $(document).on("click", ".del-button", function () {
                var x = confirm('Bạn muốn có xóa thuộc tính này?\nTất cả giá trị và biến thể sản phẩm có thuộc tính này sẽ bị xóa và không thể khôi phục. ');
                var id = $(this).data('id');
                if (x) {
                    $("#load").show();
                    var _token = $('meta[name="csrf-token"]').attr('content');
                    $.ajax({
                        type: "POST",
                        url: "{{route('backend.products.attributes.del')}}",
