...
@section('content')
    ...
    <div class="row page-titles">
        ...
        <div class="col-md-7 align-self-center">
            {{ Breadcrumbs::render('backend.booking.index') }}
        </div>
    </div>
                                    ...
                                    <table id="table_id" class="table color-table muted-table table-striped">
                                        ...
                                        @forelse($list_data as $key => $item)
                                            <tr>
                                                ...
                                                <td class="">
                                                    <span
                                                        class="badge bg-primary font_weight_bold text-white">{{$item->sort}}</span>
                                                </td>
                                                <td>
                                                    @if($item->status == 1)
                                                ...
                                                <td class="text-right">
                                                    <a href="{{Route('backend.booking.edit',[$item->id]). '?_ref=' .$current_url }}"
                                                       class="btn waves-effect waves-light btn-info btn-sm">
                                                        <i class="fa fa-pencil-square-o"></i> Sửa</a>

                                                    <a href="{{Route('backend.booking.del',[$item->id]) . '?_ref=' .$current_url }}"
