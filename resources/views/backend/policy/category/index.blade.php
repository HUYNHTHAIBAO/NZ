...
@section('content')
    ...
    <div class="row page-titles">
        ...
        <div class="col-md-7 align-self-center">
            {{ Breadcrumbs::render('backend.policy.category.index') }}
        </div>
                    ...
                    <div class="row">
                        <div class="col-md-8 offset-2">
                            <menu id="nestable-menu" style="padding: 0">
                                @if(auth()->guard('backend')->user()->can('posts.category.add'))
                                    <a href="{{Route('backend.posts.category.add')}}" class="btn flat btn-info">
                                        Thêm mới
                                    </a>
                                @endif

                                <button type="button" class="btn flat" data-action="expand-all">Mở rộng tất cả</button>
                                <button type="button" class="btn flat" data-action="collapse-all">Thu gọn tất cả
                                </button>
                            </menu>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            function sort_ajax() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('backend.posts.category.sort')}}",
                    data: dataString,
                    cache: false,
                    dataType: 'json',
            ...
            $(document).on("click", ".del-button", function () {
                var x = confirm('Xóa danh mục này?');
                var id = $(this).data('id');
                if (x) {
                    $("#load").show();
                    var _token = $('meta[name="csrf-token"]').attr('content');
                    $.ajax({
                        type: "POST",
                        url: "{{route('backend.posts.category.del')}}",
            ...
            $(document).on("click", ".edit-button", function () {
                var id = $(this).data('id');
                location.href = "{{route('backend.posts.category.edit', '')}}/" + id;
            });
        });
