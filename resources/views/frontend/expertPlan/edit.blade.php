                    ...
                    <div class="dashboard__content-wrap">
                        <div class="pb-4">
                            <p class="bg-light p-1"><a class="text-black font_weight_bold"
                                    ...
                                    href="{{route('frontend.plan.index')}}"> Danh sánh gói </a> / Cập nhật
                                gói</p>
                        </div>
                        <div class="row justify-content-center">
                            <div class="col-12 col-md-6 ">
                                <div class="dashboard__review-table p-2">
                                    <form action="{{route('frontend.plan.edit', $data->id)}}" method="post">
                                        @csrf
                                        <div class="mb-3">
                                            <input id="sort" type="text" name="sort"
                                                   value="{{old('sort', $data->sort ?? '')}}"
                                                   placeholder="Số số thứ tự hiển thị" class="form-control input_user">
                                            @if ($errors->has('sort'))
                                        ...
                                        <div class="mb-3">
                                            <textarea class="form-control input_user" name="desc" id="" rows="3"
                                                      placeholder="Mô tả">{{old('desc', $data->desc ?? '')}}</textarea>
                                            @if ($errors->has('desc'))
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input id="price" type="text" name="price_formatted"
                                                   value="{{ old('price_formatted', $data->price ?? '') }}" placeholder="Nhập giá"
                                        ...
                                        <div class="mb-3">
                                            <select class="form-select input_user"
                                                ...
                                                @foreach(\App\Models\ExpertPlan::OPTION_PLAN as $item)
                                                    <option value="{{$item['id']}}" {{ old('option_plan', $data->option_plan) == $item['id'] ? 'selected' : '' }}>{{$item['name']}}</option>
                                                @endforeach
                                        ...
                                        <div class="mb-3 d-none" id="additional_info">
                                            <input id="number_people_max" type="text" name="number_people_max"
                                                   value="{{ old('number_people_max', $data->number_people_max) }}" placeholder="Nhập số người tối đa cho nhóm"
                                                   class="form-control input_user">
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            <select class="form-select input_user" aria-label="Default select example" name="status">
                                                <option value="" {{ old('status', $data->status) === '' ? 'selected' : '' }}>Trạng thái</option>
                                                <option value="1" {{ old('status', $data->status) == 1 ? 'selected' : '' }}>Bật</option>
                                                <option value="2" {{ old('status', $data->status) == 2 ? 'selected' : '' }}>Tắt</option>
                                            </select>
